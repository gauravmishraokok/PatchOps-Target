...
def get_user():
    ...
    cursor = conn.cursor()
    query = f"SELECT username, email, role FROM users WHERE id = ?"
...
@app.route('/ping')
def ping_host():
    target_ip = request.args.get('ip') if request.args.get('ip') in ALLOWED_IPS else None
...
def deserialize_data():
    ...
    obj = pickle.loads(data, fix_imports=True, encoding='latin1')
