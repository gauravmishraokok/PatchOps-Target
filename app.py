...
output = subprocess.check_output(['ping', '-c', '1', target_ip], text=True)
...
def get_profile(user_id):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    if request.remote_user == user_id:
...
if redirect_url in whitelist:
    return redirect(redirect_url)
else:
    return jsonify({"error": "Unauthorized redirect"})
